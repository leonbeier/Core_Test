...
void pinMode(uint8_t pin, uint8_t mode)
{
	...
	if(pin < 32){
		...
		uint32_t data = IORD_ALTERA_AVALON_PIO_DIRECTION(PIO_1_BASE);
		if(mode == OUTPUT) data |= (1 << (pin-32));
		else data &= ~(1 << (pin-32));
		IOWR_ALTERA_AVALON_PIO_DIRECTION(PIO_1_BASE, data);
	}
#else
	uint32_t data = IORD_ALTERA_AVALON_PIO_DIRECTION(PIO_0_BASE);
	if(mode == OUTPUT) data |= (1 << pin);
...
void digitalWrite(uint8_t pin, uint8_t val)
{
	...
	if(pin < 32){
		...
		if(val) IOWR_ALTERA_AVALON_PIO_SET_BITS(PIO_1_BASE, 1 << (pin-32));
		else IOWR_ALTERA_AVALON_PIO_CLEAR_BITS(PIO_1_BASE, 1 << (pin-32));
	}
#else
	if(val) IOWR_ALTERA_AVALON_PIO_SET_BITS(PIO_0_BASE, 1 << pin);
	else IOWR_ALTERA_AVALON_PIO_CLEAR_BITS(PIO_0_BASE, 1 << pin);
